...
MAYBE_EXTERN int                max_multi_socket        DEFVAL
...
MAYBE_EXTERN char               remote_ip[40];
...
#ifdef USE_OPENSSL
MAYBE_EXTERN BIO              * twinSipp_bio;
MAYBE_EXTERN SSL              * twinSipp_ssl;
...
MAYBE_EXTERN struct   sipp_socket *new_sipp_socket(bool use_ipv6, int transport);
...
MAYBE_EXTERN int      sipp_bind_socket(struct sipp_socket *socket, struct sockaddr_storage *saddr, int *port);
MAYBE_EXTERN int      sipp_connect_socket(struct sipp_socket *socket, struct sockaddr_storage *dest);
MAYBE_EXTERN int      sipp_reconnect_socket(struct sipp_socket *socket);
MAYBE_EXTERN void     sipp_customize_socket(struct sipp_socket *socket);
MAYBE_EXTERN int      delete_socket(int P_socket);
...
MAYBE_EXTERN struct sipp_socket *main_socket            DEFVAL(NULL);
MAYBE_EXTERN struct sipp_socket *main_remote_socket     DEFVAL(NULL);
MAYBE_EXTERN struct sipp_socket *tcp_multiplex          DEFVAL(NULL);
...
MAYBE_EXTERN bool          is_ipv6                      DEFVAL(false);
...
MAYBE_EXTERN set<struct sipp_socket *> sockets_pending_reset;

MAYBE_EXTERN struct addrinfo *local_addr_storage;

MAYBE_EXTERN struct sipp_socket *twinSippSocket         DEFVAL(NULL);
MAYBE_EXTERN struct sipp_socket *localTwinSippSocket    DEFVAL(NULL);
...
MAYBE_EXTERN struct sipp_socket *local_sockets[MAX_LOCAL_TWIN_SOCKETS];
